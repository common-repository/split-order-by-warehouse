...
if (!class_exists('wsowsunarc_default_class')) {

    class wsowsunarc_default_class {
...
//                 create sub order //
				...
				update_post_meta($order->id,'order_status_result','Split Order');
        ...
        function sunarc_change_order_received_text($str, $order) {
            ...
            $str .= $this->wsowsunarc_change_order_received_text($order_id);
            return $str;
        }

        function wsowsunarc_change_order_received_text($order_id, $deleted = true) {
		    include('mail.php');
            ...
            $wsowsunarc_cart_notices = get_option('woo_sunarc_cart_notices', true);
            $co_total = $wsowsunarc_cart_notices['co_total'];
            ...
            if (!empty($posts_array)) {
                ...
                <section class="woocommerce-order-details child_orderss">
                    <h2 class="woocommerce-order-details-title"><?php echo $wsowsunarc_cart_notices['co_heading'] ? $wsowsunarc_cart_notices['co_heading'] : 'Split Order' . (count($posts_array) > 1 ? 's' : ''); ?></h2>
                    <?php
                    ...
                    foreach ($posts_array as $post_data) {
                        ...
                        <h3 class="child_order_heading"><?php echo $wsowsunarc_cart_notices['co_number'] ? $wsowsunarc_cart_notices['co_number'] : 'Order number'; ?> <?php echo $post_data; ?></h3>
	...
	public function change_formatted_meta_data($orders) {
        ...
        public function sunarc_thank_you_page_template($template, $template_name, $template_path) {
            if ('checkout/thankyou.php' == $template_name) {
                $template = wsowsunarc_sunarc_plugin_dir . '/include/checkout/thankyou.php';
            }
            return $template;
